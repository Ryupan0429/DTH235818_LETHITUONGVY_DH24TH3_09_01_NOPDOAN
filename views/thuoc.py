...
from features.thuoc_dialog import ThuocDialog 
...
class ThuocTab(tk.Frame):
    ...
    def _on_edit(self):
        ...
        if not sel:
            messagebox.showinfo("Sửa", "Vui lòng chọn một loại thuốc để sửa.")
            return
            
        try:
            item = self.tree.item(sel[0])
            mathuoc_idx = DISPLAY_COLS.index("MaThuoc")
            mathuoc = item["values"][mathuoc_idx]
            
            ThuocDialog(self, mathuoc)
            self.load_data()
            
        except (ValueError, IndexError):
            messagebox.showerror("Lỗi", "Không thể xác định Mã Thuốc.")

    def _on_delete(self):
        sel = self.tree.selection()
        if not sel:
            messagebox.showinfo("Xóa", "Vui lòng chọn một loại thuốc để xóa.")
            return

        try:
            item = self.tree.item(sel[0])
            mathuoc_idx = DISPLAY_COLS.index("MaThuoc")
            tenthuoc_idx = DISPLAY_COLS.index("TenThuoc")
            mathuoc = item["values"][mathuoc_idx]
            ten_thuoc = item["values"][tenthuoc_idx] 
            
            if not messagebox.askyesno("Xác nhận", f"Bạn có chắc muốn xóa thuốc:\n{ten_thuoc} ({mathuoc})?", parent=self):
                return
                
            conn = get_connection()
            cur = conn.cursor()
            cur.execute("DELETE FROM dbo.ThuocNongDuoc WHERE MaThuoc = ?", (mathuoc,))
            conn.commit()
            conn.close()
            
            messagebox.showinfo("Thành công", "Đã xóa thuốc thành công.", parent=self)
            self.load_data()

        except (ValueError, IndexError):
            messagebox.showerror("Lỗi", "Không thể xác định Mã Thuốc.")
        except Exception as e:
            messagebox.showerror("Lỗi CSDL", f"Không thể xóa. Có thể thuốc đã được sử dụng trong hóa đơn.\n{e}", parent=self)
    ...
    def _on_heading_click(self, col):
        # Bỏ giới hạn, cho phép sort tất cả các cột
        ...
        try:
            ...
            if kw:
                ...
                for col in DISPLAY_COLS:
                    if col.lower() == 'dongia':
                        # Dùng CAST để tìm kiếm LIKE trên cột số
